...
include ('conexion.php');
   ...
   function(){ alertify.message('OK'); window.location= 'vssolicitudes.php?id=".$usuario."'; }); </script>";
    ...
    $terminados=mysqli_query($conexion,"select sum(porcentaje) from avances where ID_CONTRA = $id and ESTADO='Aprobado'") or
    die("Problemas en el select:".mysqli_error($conexion));

    if($ter =mysqli_fetch_array($terminados)){

   if($ter['sum(porcentaje)'] == 100 and $estado == 'terminar'){
